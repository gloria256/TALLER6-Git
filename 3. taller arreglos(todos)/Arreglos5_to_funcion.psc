...
Proceso Arreglos5_to_funcion
	Definir matriz,fila Como Caracter;
	Definir vector0_9 Como Entero;
	Definir i,j,cont,cont2,row,col Como Entero;
	...
	para i<-0 hasta 11 Con Paso 1 Hacer
		cont2<-0;
		modificarMatriz(i,matriz,vector0_9,cont,cont2);
	FinPara
	
	cont <-0;
	fila <- "";
	para i<-0 hasta 11 Con Paso 1 Hacer
		formatoSalida(matriz,i,i=0);
		formatoSalida(matriz,i,i=1);
		formatoSalida(matriz,i,i>=2);
	FinPara
	
	mientras tag = Verdadero Hacer
		capturarInformacion("inserte fila",row);
		capturarInformacion("inserte columna",col);
		escribir (row+1)*(col+1);
	FinMientras
FinProceso	


SubProceso inicializarMatriz(matriz Por Referencia,valorRellenar)
	Definir i Como Entero;
	Definir j Como Entero;
	para i<-0 hasta 11 Con Paso 1 Hacer
		para j<-0 hasta 52 Con Paso 1 Hacer
			matriz[i,j]<- valorRellenar;
		FinPara
	FinPara
FinSubProceso

SubProceso formatoSalida(matriz Por Referencia,i,condicion)
	Definir j Como Entero;
...
SubProceso modificarMatriz(i,matriz Por Referencia,vector0_9,cont Por Referencia,cont2 Por Referencia)
	Definir j Como Entero;
	para j<-0 hasta 52 Con Paso 1 Hacer
